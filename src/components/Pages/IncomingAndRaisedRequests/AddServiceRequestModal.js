...
function getModalTitle(isGuestRequest) {
  ...
  return `Add Bulk Department Request`
}
...
function AddServiceRequestModal({ type, visible, hideModal }) {
  const {
    hotelInfo,
    userInfo,
    currentLanguage,
  ...
  const frontDeskDepartments = departmentsNew
    .map(d => ({
      ...d,
      name: d.name,
      value: d.id,
    }))
    .filter(d => d.key === 'front-desk')
    .filter(d => d.active)

  const services = servicesNew[frontDeskDepartments[0].id]
    ?.map(s => ({
      ...s,
      name: s.name,
      value: s.id,
    ...
    .filter(s => s.key === 'checkout-and-request-bill')
  ...
  function SelectDepartment() {
    if (selectedService && sortedDepartments?.length) {
      return (
        <div className='col-12 col-sm-12'>
          <div className='form-group cmn-input'>
            <Form.Item
    ...
    return (
      <div className='col-12 col-sm-12'>
        <div className='form-group cmn-input'>
          ...
          >
            <Input
              onChange={e => onChange('selectedService', e.target.value)}
  ...
  async function saveRequest(anotherRequestConfirmed = false) {
    try {
      ...
      const requestDataArray = [] // Array to hold request data for each department

      // Iterate over each selected department
      for (const departmentId of selectedDepartment) {
        const departmentInfo =
          departmentAndServiceIdToInfo?.[departmentId] ?? {}
        ...
        const { errorMessage, waitingForConfirmation } = await validateRequest({
          ...
          noOfRequestsAllowed,
          requestedTime,
          selectedDepartment: [departmentId], // Pass only the current department for validation
          selectedService,
          serviceKey,
          setConfirmationMessage,
          setConfirmButtonText,
        ...
        if (images.length > 0) {
          requestData.beforeStartUploadById = userInfo.id || userInfo.userId
          requestData.beforeStartUploadByName = userInfo.name
          requestData.beforeStartUploadImages = images
        }
        ...
        requestData.locationName = locationName
        requestData.locationTypeId = locationTypeId
        requestData.locationTypeName = locationTypeName
        requestData.locationTypeisDefault = locationTypeisDefault
        requestData.isLocationUsed = isLocationUsed
        requestData.isRecurring = false
        ...
        requestDataArray.push({ guestList, notificationData, requestData })
      }

      for (const requestDataObj of requestDataArray) {
        const { guestList, notificationData, requestData } = requestDataObj
