...
import StaffHelper from './Components/StaffHelper'
import SelectOption from '../../Common/SelectOption/SelectOption'
import { pdf } from '@react-pdf/renderer'
import { saveAs } from 'file-saver'
import ExportPdf from './Components/HelperComponents/ExportPdf'
import { ExportExcel } from './Components/HelperComponents/ExportExcel'
import {
  chartConfigurations,
  generateChart,
  mergeCanvases,
} from '../../../services/Reports'
import { fetchAllHotelLocations } from '../../../services/location'
...
export const ReportState = createContext()

export const ReportStateProvider = props => {
  ...
  useEffect(() => {
    setFilteredLocations(allLocationsReports)
  }, [allLocationsReports])

  useEffect(() => {
    if (hotelId) {
      fetchAllHotelLocations({ dispatch, hotelId })
    }
  }, [hotelId, dispatch])
  ...
  const usageReportVal = useCallback(async () => {
    if (repId === 6) {
      // Only fetch usage report for the Usage Report page
  ...
  }, [hotelId, filteredStartDate, groupBy, filteredEndDate, repId])
  ...
  const exportPdf = ({ fileName, column, data }) => {
    ...
    try {
      ...
      const doc = ExportPdf({ column, data: newData, profileData })
      doc.save(`${profileData.reportTitle}.pdf`)
    } catch (error) {
