...
export const AddRestaurantReservationListener = ({ hotelId, dispatch }) => {
  try {
    let collectionKey = `AddRestaurantReservationListener${Collections.REQUEST_INFO_DEPARTMENT_REQUEST}`
    if (!hotelId || unsubscribeList[collectionKey]) return
    let servicekey =
    ...
    let sub = db
      ...
      .onSnapshot(snapshot => {
        const requests = GetDataFromSnapshot(snapshot)
        requests.sort(sortByCreatedAt)
        dispatch(actions.setRestaurantReservations(requests))
        dispatch(actions.setLoadingRestaurantReservationServices(false))
      })
    unsubscribeList[collectionKey] = sub
  } catch (error) {
    console.log(error)
